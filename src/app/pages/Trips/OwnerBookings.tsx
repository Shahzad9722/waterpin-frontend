...
export function OwnerBookings() {
  ...
  const [bookings, set_bookings] = React.useState<any>(null);
  ...
  React.useEffect(() => {
    ...
    setTimeout(() => dispatch(actions.getAllBookings()), 0);
  },[])

  React.useEffect(() => {

    if (allBookings !== undefined && allBookings !== null) {

      let filteredItems:any = []

      switch (activeBar !== -1) {
        case activeBar === 0:
          filteredItems =  allBookings.filter((trip:any) => (trip.status === "Pending" || trip.status === "Confirmed"))
          break;
        case activeBar === 1:
          filteredItems =  allBookings.filter((trip:any) => trip.status === "Completed")
          break;
        case activeBar === 2:
          filteredItems =  allBookings.filter((trip:any) => trip.status === "Cancelled")
          break;
        default:
          filteredItems = allBookings
          break;
      }

      set_bookings(filteredItems)
      set_tripLoading(false)
    }
  },[allBookings, activeBar])
  ...
  return (
    <>
      ...
      <Nav/>
      <BookingDetail trip={tripDetail} open={tripDetailActive} onClose={toggleTripDetailModal}/>
      <Container>
        <TopHeader>
        <h1>My Bookings</h1>
        </TopHeader>
                ...
                ? (
                  <>
                    {bookings.map((item,index)=>(
                            <TripCard view={'owner'} trip={item} index={index} clk={toggleTripDetailModal}/>
                     ))
                    }
                  </>
                )
